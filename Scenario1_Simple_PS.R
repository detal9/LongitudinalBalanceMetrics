##################################################################################################################################
#  Scénario 1 de l'article de Franklin en utilisant le DAG de l'article
...
# Covariables et exposition au temps t=0
...
M0 = rlnorm(n,0,0.5); 
N0 = rnorm(n,0,10) ; 
O0 = rbinom(n,1,plogis(-0.05+2*L0)); 
P0 = rbinom(n,1,0.2); 
Q0 = sample(c(1,2,3,4,5),n,replace=TRUE,c(0.5,0.3,0.1,0.05,0.05)); 
A0 = rbinom(n,1,plogis(-1.3+0.05*L0+0.05*M0+0.1*N0+0.75*O0+0.5*P0+0.4*Q0));

# Covariables et exposition au temps t=1

L1 = -A0+rnorm(n,0,1);
M1 = -0.5*A0+rlnorm(n,0,0.5);
N1 = -0.25*A0+rnorm(n,0,10);
O1 = rbinom(n,1,plogis(1.23+2*L1-0.5*A0));
P1 = rbinom(n,1,plogis(-1.1-0.75*A0));
Q1 = sample(c(1,2,3,4,5),n,replace=TRUE,c(0.4,0.3,0.2,0.05,0.05));
A1 = rbinom(n,size=1,plogis(-1.7+0.69*A0+0.05*L1+0.05*M1+0.1*N1+0.75*O1+0.5*P1+0.4*Q1));

Y = rbinom(n,size=1,plogis(-4.3-0.69*A0+0.5*L0+0.5*M0+0.05*N0
                           +O0+P0+0.2*Q0-0.69*A1+0.5*L1+0.5*M1
                           +0.05*N1+O1+P1+0.2*Q1));

# Calcul des vraies valeurs de paramètres

L1.0 =  0+rnorm(n,0,1);
L1.1 = -1+rnorm(n,0,1);
M1.0 = -0.5*0+rlnorm(n,0,0.5);
M1.1 = -0.5*1+rlnorm(n,0,0.5);
N1.0 = -0.25*0+rnorm(n,0,10);
N1.1 = -0.25*1+rnorm(n,0,10);
O1.0 = rbinom(n,1,plogis(1.23+2*L1.0-0.5*0));
O1.1 = rbinom(n,1,plogis(1.23+2*L1.1-0.5*1));
P1.0 = rbinom(n,1,plogis(-1.1-0.75*0));
P1.1 = rbinom(n,1,plogis(-1.1-0.75*1));


Y.00 = rbinom(n,size=1,plogis(-4.3-0.69*0+0.5*L0+0.5*M0+0.05*N0
                              +O0+P0+0.2*Q0-0.69*0+0.5*L1.0+0.5*M1.0
                              +0.05*N1.0+O1.0+P1.0+0.2*Q1));

Y.01 = rbinom(n,size=1,plogis(-4.3-0.69*0+0.5*L0+0.5*M0+0.05*N0
                              +O0+P0+0.2*Q0-0.69*1+0.5*L1.0+0.5*M1.0
                              +0.05*N1.0+O1.0+P1.0+0.2*Q1));

Y.10 = rbinom(n,size=1,plogis(-4.3-0.69*1+0.5*L0+0.5*M0+0.05*N0
                              +O0+P0+0.2*Q0-0.69*0+0.5*L1.1+0.5*M1.1
                              +0.05*N1.1+O1+P1.1+0.2*Q1));

Y.11 = rbinom(n,size=1,plogis(-4.3-0.69*1+0.5*L0+0.5*M0+0.05*N0
                              +O0+P0+0.2*Q0-0.69*1+0.5*L1.1+0.5*M1.1
                              +0.05*N1.1+O1.1+P1.1+0.2*Q1));
...
for(i in 1:B){
  ...
  l_1 = as.numeric(-a_0+rnorm(n,0,1));
  m_1 = as.numeric(-0.5*a_0+rlnorm(n,0,0.5));
  n_1 = as.numeric(-0.25*a_0+rnorm(n,0,10));
  ...
  Y = as.numeric(rbinom(n,size=1,plogis(-4.3-0.69*a_0+0.5*l_0+0.5*m_0
                                        +0.05*n_0+o_0+p_0+0.2*q_0
                                        -0.69*a_1+0.5*l_1+0.5*m_1+0.05*n_1
                                        +o_1+p_1+0.2*q_1)));
  
  ### Calcul des termes pour les poids stabilisé marginal
  
  #  Dénominateur du poids
  
  mod.dnom1 = glm(
    a_0~l_0+m_0+n_0+o_0+p_0+q_0,
    family=binomial(link = "logit"));
  
  dnom1 =ifelse(
    a_0== 0, 1-predict(mod.dnom1, type = "response"),
    predict(mod.dnom1, type = "response"));
  
  mod.dnom2 = glm(
    a_1~l_0+m_0+n_0+o_0+p_0+q_0+l_1+m_1+n_1+o_1+p_1+q_1+a_0,
    family=binomial(link = "logit"));
  
  dnom2=ifelse(
    a_1== 0, 1-predict(mod.dnom2, type = "response"),
    predict(mod.dnom2, type = "response"));
  
  #  Numérateur du poids
  
  mod.num1 = glm(
    a_0~1,family=binomial(link="logit"));
  
  num1 = ifelse(
    a_0== 0, 1-predict(mod.num1, type = "response"),
    predict(mod.num1, type = "response"));
  
  mod.num3=glm(
    a_1~1,family=binomial(link="logit"));
  
  num3 = ifelse(
    a_1== 0, 1-predict(mod.num3, type = "response"),
    predict(mod.num3, type = "response"));
  
  #summary(num1[a_0 == 1]/dnom1[a_0 == 1])
  #summary(num3[a_1 == 1]/dnom2[a_1 == 1])
  #summary(dnom1[a_0 == 1])
  #summary(dnom2[a_1 == 1])
  ...
  LD=c(ld(data.l,6,7,5),ld(data.m,6,7,5),ld(data.n,6,7,5),
       ld(data.o,6,7,5),ld(data.p,6,7,5),ld(data.q,6,7,5),
       ld(data1.l,6,7,5),ld(data2.l,6,7,5),ld(data1.m,6,7,5),
       ld(data2.m,6,7,5),ld(data1.n,6,7,5),ld(data2.n,6,7,5),
  ...
  mod.dnom1=glm(
    a_0~l_0+m_0+n_0+o_0+p_0+q_0,data = data.a,
    family=binomial(link="logit"));
  
  mod.dnom2=glm(
    a_1~l_0+m_0+n_0+o_0+p_0+q_0+l_1+m_1+n_1+o_1+p_1+q_1,
  ...
  lmfit=glm(
    as.factor(Y)~as.factor(a_0)+as.factor(a_1),
    data=data.a,family=binomial(link="logit"));
  ...
  mod.dnom1=glm(
    a_0~l_0+m_0+n_0+o_0+p_0+q_0,data = data.a,
    weights=wtx_0,family=binomial(link="logit"));
  
  mod.dnom2=glm(
    a_1~l_0+m_0+n_0+o_0+p_0+q_0+l_1+m_1+n_1+o_1+p_1+q_1,
    data = data.a,weights=wtx_1,family=binomial(link="logit"));
  ...
  lmfit=glm(
    as.factor(Y)~as.factor(a_0)+as.factor(a_1),
    weights=wtx_1,data=data.a,family=binomial(link="logit"));
  ...
  mod.dnom1=glm(
    a_0~l_0+m_0+n_0+o_0+p_0+q_0,data = data.a,
    weights=wtx1_0,family=binomial(link="logit"));
  ...
  lmfit=glm(
    as.factor(Y)~as.factor(a_0)+as.factor(a_1),
    weights=wtx1_1,data=data.a,family=binomial(link="logit"));
...
mean.Stat.CT=c(mean.Stat.C,mean.Stat.C3,mean.Stat.C6,mean.Stat.C9,mean.Stat.C12,mean.Stat.C15) 
mean.Stat.CT1=c(mean.Stat.C1,mean.Stat.C4,mean.Stat.C7,mean.Stat.C10,mean.Stat.C13,mean.Stat.C16) 
mean.Stat.CT2=c(mean.Stat.C2,mean.Stat.C5,mean.Stat.C8,mean.Stat.C11,mean.Stat.C14,mean.Stat.C17) 
...
save(mean.BiaisT,mean.BiaisT1,mean.BiaisT2,meanDT,meanDT1,meanDT2,
     meanSMDT, meanSMDT1, meanSMDT2,meanOVLT,meanOVLT1,meanOVLT2,
     meanKST,meanKST1,meanKST2,meanLDT,meanLDT1,meanLDT2,
     meanStat.CT,meanStat.CT1,meanStat.CT2,meanMHBT,meanMHBT1,meanMHBT2,
     meanGWDT,meanGWDT1,meanGWDT2,BiaisT,BiaisT1,BiaisT2,mean.DT,mean.DT1,
     mean.DT2,mean.SMDT,mean.SMDT1,mean.SMDT2,mean.OVLT,mean.OVLT1,
     mean.OVLT2,mean.KST,mean.KST1,mean.KST2,
     mean.LDT,mean.LDT1,mean.LDT2,mean.Stat.CT,mean.Stat.CT1,mean.Stat.CT2,
     mean.MHBT,mean.MHBT1,mean.MHBT2,mean.GWDT,mean.GWDT1,mean.GWDT2
     ,file = "Essaie3")
