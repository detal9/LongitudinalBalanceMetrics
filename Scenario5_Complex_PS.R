...
n_0 = as.numeric(rnorm(n,0,10)) ; # age
...
a_0 = as.numeric(rbinom(n,1,plogis(-0.05+0.01*l_0+0.01*m_0+0.02*n_0+
                                     0.02*o_0+0.01*p_0+0.01*q_0)));

l_1 = as.numeric(rnorm(n,2,1));
m_1 = as.numeric(rlnorm(n,2,0.5));
n_1 = as.numeric(rnorm(n,2,10));
o_1 = as.numeric(rbinom(n,1,plogis(-4.5+2*l_1)))
p_1 = as.numeric(rbinom(n,1,0.3));
q_1 = as.numeric(sample(c(1,2,3,4,5),n,replace=TRUE,c(0.4,0.3,0.2,0.05,0.05)));

a_1 = as.numeric(rbinom(n,size=1,plogis(-0.2+0.01*l_1+0.01*m_1+
                                          0.02*n_1+0.02*o_1+0.01*p_1+0.01*q_1)));

Y = as.numeric(rbinom(n,size=1,plogis(-20.5+l_0+m_0+0.1*n_0+2*o_0+2*p_0+0.4*q_0
                                      +l_1+m_1+0.1*n_1+2*o_1+2*p_1+0.4*q_1)));

Y.00 = Y.01 = Y.10 =Y.11 = Y
...
for(i in 1:B){
  ...
  a_1 = as.numeric(rbinom(n,size=1,plogis(-0.2+0.01*l_1+0.01*m_1+0.02*n_1+0.02*o_1+0.01*p_1+0.01*q_1)));
